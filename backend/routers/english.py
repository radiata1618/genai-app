...
def create_preparation(req: PreparationRequest, db: firestore.Client = Depends(get_db)):
    ...
    prompt = f"""
    あなたは、実用性の高い英語表現を指導するプロの英語コーチです。
    生徒は「{req.topic}」というトピックについて、単に難しい単語を知りたいのではなく、
    **「実際の会話で自在に使いこなせる（Active Vocabulary）」**表現を増やしたいと考えています。
    ...
    **可読性を高めるため、各セクションや項目の間には必ず空行を入れてください。**
    ...
    1. **実践的語彙グループとコロケーション (Practical Vocabulary Groups & Collocations)**
       - 単語を単体で列挙するのではなく、**関連する語彙群（名詞・形容詞・動詞のセット）**や**文脈ごとのまとまり**として提示してください。
       - **形式例**:
         - **[Category Name/Pattern]**:
           - 構文: *"It was a [Adjective] [Noun]."*
           - **入れ替え可能な形容詞**: *Productive, Fruitful, Constructive* (解説: それぞれのニュアンスの違い)
           - **入れ替え可能な名詞**: *Meeting, Session, Discussion*
           - **関連イディオム**: *Cover a lot of ground* (意味: 幅広く議論する)

       - このように、「セットで覚える」「文脈の中で置き換え可能」な形で、トピックに関連する語彙を3〜4グループ紹介してください。

    2. **重要フレーズと応用バリエーション (Key Phrases & Variations)**
       - トピック頻出のキーフレーズを提示し、その**言い換え表現（Formal/Casual）**や、**一部を入れ替えた応用パターン**を並記してください。
       - **形式**:
         - **Core Phrase**: *"phrase..."*
         - **Variations**:
           - *Formal*: "..."
           - *Casual*: "..."
         - **Substitution**: "..." の部分を "A", "B" に入れ替え可能。

    3. **会話シナリオ (Conversation Scenario)**
       - 上記の語彙・フレーズを自然に盛り込んだ対話。
...
def chat_with_context(req: ChatRequest):
    ...
    try:
        ...
        2. **極めて簡潔に**: 回答はサイドバー（幅の狭い画面）に表示されます。スクロールせずに読めるよう、**1画面（約20行、500文字以内）**に収まるように要点を絞って短くてください。
