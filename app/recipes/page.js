...
export default function RecipeListPage() {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 p-6 overscroll-none">
            <header className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800">Recipes</h1>
                    <p className="text-slate-500 mt-1">Manage your cooking collection</p>
                </div>
                <Link href="/recipes/new">
                    <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-full font-medium shadow-lg transition-all transform hover:scale-105 active:scale-95 flex items-center">
                        <span className="mr-2 text-xl">+</span> Add Recipe
                    </button>
                </Link>
            </header>
            ...
            <div className="mb-8 relative group">
                <input
                    ...
                    className="w-full p-4 pl-12 rounded-2xl border-2 border-slate-200 bg-white text-slate-700 shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all outline-none"
                />
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                    🔍
                </span>
            </div>
                                ...
                                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                                    ...
                                    <div className="flex gap-4 mb-3">
                                        <div className="flex items-center gap-1">
                                            <span className="text-[10px] font-bold text-slate-400 uppercase">Effort</span>
                                            <StarRating value={recipe.effort_rating} sizeClass="text-xs" colorClass="text-blue-400" />
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <span className="text-[10px] font-bold text-slate-400 uppercase">Taste</span>
                                    ...
                                    <div className="mt-auto pt-4 border-t border-slate-100 flex justify-end items-center text-indigo-500 font-medium opacity-0 group-hover:opacity-100 transition-opacity">
                                        View Details →
                                    </div>
                                </div>
                            </Link>
